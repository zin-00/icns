...
// Pathfinding graph data
const pathfindingNodes = ref(new Map()); // nodeId -> {lat, lng, type: 'private'|'entry'|'exit'}
const pathfindingEdges = ref(new Map()); // nodeId -> [{nodeId, cost}]
...
class PriorityQueue {
    constructor() { this.heap = []; }
        enqueue(node, priority) { this.heap.push({ node, priority }); this.heap.sort((a, b) => a.priority - b.priority); }
        dequeue() { return this.heap.shift(); }
        isEmpty() { return this.heap.length === 0; }
        contains(node) { return this.heap.some(element => element.node === node); }
}
...
// Create pathfinding graph from GeoJSON data
const createPathfindingGraph = () => {
  if (!privateRoutes.value?.features) return;

  const nodes = new Map();
  const edges = new Map();
  let nodeIdCounter = 0;

  console.log('🔄 Creating pathfinding graph from GeoJSON data...');

  // Process each LineString feature to create nodes and edges
  privateRoutes.value.features.forEach((feature, featureIndex) => {
    if (feature.geometry?.type === 'LineString') {
      const coordinates = feature.geometry.coordinates;
      const pathNodes = [];

      console.log(`Processing feature ${featureIndex} with ${coordinates.length} coordinates`);
      ...
      coordinates.forEach((coord, index) => {
        ...
        // Determine node type - entry points are at the start/end of paths
        let nodeType = 'private';
        if (index === 0 || index === coordinates.length - 1) {
          nodeType = 'entry'; // Entry/exit points for connecting to public roads
        }
        ...
        nodes.set(nodeId, {
          ...
          type: nodeType,
          originalIndex: index,
          featureId: feature.id || `feature_${featureIndex}`
        });
      ...
      for (let i = 0; i < pathNodes.length - 1; i++) {
        ...
        if (!edges.has(nextNodeId)) edges.set(nextNodeId, []);

        edges.get(currentNodeId).push({ nodeId: nextNodeId, cost: distance });
        edges.get(nextNodeId).push({ nodeId: currentNodeId, cost: distance });
      }
    }
  });
  ...
  console.log(`🗺️ Created pathfinding graph with ${nodes.size} nodes and ${edges.size} edge groups`);
  
  // Log entry points for debugging
  const entryPoints = Array.from(nodes.entries()).filter(([_, node]) => node.type === 'entry');
  console.log(`📍 Found ${entryPoints.length} entry/exit points:`, entryPoints.slice(0, 5));
};

// Find nearest graph node and return its ID and distance
const findNearestNode = (lat, lng, nodeType = null) => {
...
const findOptimalPath = async (startLat, startLng, endLat, endLng) => {
  if (pathfindingNodes.value.size === 0) {
    console.warn('Pathfinding graph not initialized');
    return null;
  }

  console.log(`🔍 Finding optimal path from [${startLat}, ${startLng}] to [${endLat}, ${endLng}]`);

  // Find nearest entry points to private network
  const nearestEntry = findNearestNode(startLat, startLng, 'entry');
  const nearestExit = findNearestNode(endLat, endLng, 'entry');

  if (!nearestEntry.nodeId || !nearestExit.nodeId) {
    console.log('❌ No suitable entry/exit points found, using direct routing');
    return null;
  }

  const entryNode = pathfindingNodes.value.get(nearestEntry.nodeId);
  const exitNode = pathfindingNodes.value.get(nearestExit.nodeId);

  console.log(`📍 Entry point: [${entryNode.lat}, ${entryNode.lng}] (${nearestEntry.distance.toFixed(2)}m away)`);
  console.log(`📍 Exit point: [${exitNode.lat}, ${exitNode.lng}] (${nearestExit.distance.toFixed(2)}m away)`);

  // Check if using private path would be beneficial
  const directDistance = calculateDistance(startLat, startLng, endLat, endLng);
  const privatePathDistance = 
    calculateDistance(startLat, startLng, entryNode.lat, entryNode.lng) +
    calculateDistance(entryNode.lat, entryNode.lng, exitNode.lat, exitNode.lng) +
    calculateDistance(exitNode.lat, exitNode.lng, endLat, endLng);

  console.log(`📏 Direct distance: ${directDistance.toFixed(2)}m`);
  console.log(`📏 Private path distance: ${privatePathDistance.toFixed(2)}m`);

  // Only use private path if it's not significantly longer (allow 30% overhead for campus access)
  if (privatePathDistance > directDistance * 1.3) {
    console.log('🚫 Direct route is more efficient than private path');
    return null;
  }

  // Use A* to find path through private network
  const astar = new EnhancedAStar(pathfindingNodes.value, pathfindingEdges.value);
  const privatePath = astar.findPath(nearestEntry.nodeId, nearestExit.nodeId);

  if (!privatePath) {
    console.log('❌ No path found through private network');
    return null;
  }

  console.log(`✅ Found private path with ${privatePath.length} nodes`);

  // Build complete route
  const fullRoute = [];
  
  // 1. Public route to entry point
  console.log('🔄 Getting public route to entry point...');
  const publicToEntry = await getPublicRoute(
    { lat: startLat, lng: startLng },
    { lat: entryNode.lat, lng: entryNode.lng }
  );
  
  if (publicToEntry && publicToEntry.coordinates) {
    publicToEntry.coordinates.forEach(coord => fullRoute.push(coord));
  } else {
    fullRoute.push([startLat, startLng]);
    fullRoute.push([entryNode.lat, entryNode.lng]);
  }

  // 2. Private path through campus
  console.log('🏫 Adding private campus path...');
  privatePath.forEach(nodeId => {
    const node = pathfindingNodes.value.get(nodeId);
    fullRoute.push([node.lat, node.lng]);
  });

  // 3. Public route from exit to destination
  console.log('🔄 Getting public route from exit point...');
  const exitToPublic = await getPublicRoute(
    { lat: exitNode.lat, lng: exitNode.lng },
    { lat: endLat, lng: endLng }
  );

  if (exitToPublic && exitToPublic.coordinates) {
    // Skip first point to avoid duplication
    exitToPublic.coordinates.slice(1).forEach(coord => fullRoute.push(coord));
  } else {
    fullRoute.push([endLat, endLng]);
  }

  console.log(`✅ Complete route created with ${fullRoute.length} points`);
  ...
  return {
    ...
    exitPoint: exitNode,
    distance: privatePathDistance,
    pathSegments: {
      publicToEntry: publicToEntry?.coordinates?.length || 1,
      privatePath: privatePath.length,
      exitToPublic: exitToPublic?.coordinates?.length || 1
    }
  };
};
...
const generateInstructions = (route, isPrivatePath, pathSegments = null) => {
  const instructions = []
  ...
  if (isPrivatePath && pathSegments) {
    // More detailed instructions for mixed public/private routes
    const publicToEntryEnd = pathSegments.publicToEntry;
    const privatePathEnd = publicToEntryEnd + pathSegments.privatePath;
    ...
    for (let i = 0; i < route.length - 1; i++) {
      ...
      // Instructions based on route segments
      if (i === Math.floor(publicToEntryEnd * 0.5)) {
        instructions.push({
          icon: '🛣️',
          text: 'Continue on public road toward campus',
          distance: `${Math.round(cumulativeDistance)} m`
        })
      }
      
      if (i === publicToEntryEnd - 1) {
        instructions.push({
          icon: '🏫',
          text: 'Enter campus - follow private path',
          distance: `${Math.round(cumulativeDistance)} m`
        })
      }
      
      if (i === privatePathEnd - 1) {
        instructions.push({
          icon: '🚪',
          text: 'Exit campus to public road',
          distance: `${Math.round(cumulativeDistance)} m`
        })
      }
    }
  } else {
    // Standard instructions for public-only routes
    for (let i = 0; i < route.length - 1; i++) {
      const segmentDist = calculateDistance(
        route[i][0], route[i][1],
        route[i + 1][0], route[i + 1][1]
...
// Main routing function with enhanced pathfinding
const createRoute = async (clickLatLng) => {
  ...
  console.log(`🎯 Creating route from [${userPos.lat}, ${userPos.lng}] to [${destPos.lat}, ${destPos.lng}]`);
  ...
  // First, try to find optimal path using pathfinding algorithm
  const optimalPath = await findOptimalPath(userPos.lat, userPos.lng, destPos.lat, destPos.lng);
  
  if (optimalPath) {
    console.log('🎉 Using optimal path with private campus routes');
    fullRoute = optimalPath.coordinates;
    isPrivatePath = true;
    pathSegments = optimalPath.pathSegments;
    
    // Update final destination to the actual end of the route
    const routeEnd = fullRoute[fullRoute.length - 1];
    finalDestination = { lat: routeEnd[0], lng: routeEnd[1] };
  } else {
    // Fallback to original logic for direct routes or when pathfinding fails
    console.log('📍 Using fallback routing logic');
    
    // Check if clicked on LineString (private path)
    const clickedFeature = findClickedLineString(clickLatLng, privateRoutes.value.features, 30);

    if (clickedFeature) {
      console.log('🏫 Routing via clicked private path');
      isPrivatePath = true;
      ...
      if (publicSegment) {
        publicSegment.coordinates.forEach(p => fullRoute.push([p[0], p[1]]));
      } else {
      ...
      const snappedDest = nearestToClick.coord;
      fullRoute.push([snappedDest[1], snappedDest[0]]);
      finalDestination = { lat: snappedDest[1], lng: snappedDest[0] };

      console.log('🎯 Private path snapped destination:', finalDestination);
    } else {
      console.log('🛣️ Public roads only');

      const publicRoute = await getPublicRoute(userPos, destPos);

      if (publicRoute) {
        publicRoute.coordinates.forEach(p => fullRoute.push([p[0], p[1]]));
        const routeEnd = publicRoute.coordinates[publicRoute.coordinates.length - 1];
        finalDestination = { lat: routeEnd[0], lng: routeEnd[1] };
        console.log('🎯 Public route snapped destination:', finalDestination, 'vs original click:', clickLatLng);
      } else {
        fullRoute.push([userPos.lat, userPos.lng]);
  ...
  // Draw route using fullRoute
  const routeColor = isPrivatePath ? '#2196F3' : '#4CAF50';
  ...
  // Place marker at final destination
  ...
  const routeTypeText = isPrivatePath 
    ? (pathSegments ? '🔄 Optimal: Public → Private → Public' : '🟢 Public → 🏫 Private')
    : '🟢 Public Roads Only';
    
  routeInfo.value = `${routeTypeText}\n📏 ${distanceText}\n⏱️ ${eta.minutes} min\n🕐 Arrive at ${eta.arrivalTime}`;

  navigationInstructions.value = generateInstructions(fullRoute, isPrivatePath, pathSegments);
  showInstructions.value = true;

  // Fit to route bounds
  map.value.fitBounds(routePolyline.value.getBounds(), { padding: [50, 50] });
  
  console.log(`✅ Route created: ${fullRoute.length} points, ${distanceText}, ${isPrivatePath ? 'with' : 'without'} private paths`);
};
...
const loadPrivateRoutes = async () => {
  try {
    ...
    // Create pathfinding graph after loading routes
...
const refreshPrivateRoutesOnMap = () =>{
    ...
    // Recreate pathfinding graph with updated data
    createPathfindingGraph();
...
const saveFeedback = async () => {
    ...
    try {
        ...
        toast.success(message.value || 'Feedback added');
